...
class GenericTemplateGeneratorApp1(TemplateGenerator):
    ...
    def generate_templates(self, texts_input, relevant_tags, n_masks=2, ranked_words_count=2):
        ...
        instances_to_write = instances_with_score_and_tag(instances)

        write_sentences("app1_instancesP1.txt", instances_to_write)
        ...
        write_sentences("app1_sentencesP2.txt", [x.original_text for x in sentences1])
        ...
        write_sentences("app1_sentencesP3.txt", write_sentencesp3)
        ...
        write_sentences("app1_sentencesP4.txt", [x.template_text for x in sentences3])
...
def instances_with_score_and_tag(instances):
    instances_to_write = list()
    for inst in instances:
        sorted_tokens_tuples = ' '.join([f"({token.word}, {token.tag}, {token.rank_score:.7f})" for token in inst.sorted_tokens])
        tokens_tuples = ' '.join([f"({token.word}, {token.tag}, {token.rank_score:.7f})" for token in inst.tokens])
                                        
        instances_to_write.append(inst.original_text + "\n" + tokens_tuples + "\n" + sorted_tokens_tuples + "\n")
    return instances_to_write
...
class GenericTemplateGeneratorApp2(TemplateGenerator):
    ...
    def generate_templates(self, texts_input, relevant_tags, n_masks=2, ranked_words_count=2):
        ...
        instances_to_write = instances_with_score_and_tag(instances)
        write_sentences("app2_instancesP1.txt", instances_to_write)
        ...
        write_sentences("app2_sentencesP2.txt", [x.original_text for x in sentences])
        ...
        write_sentences("app2_sentencesP3.txt", write_sentencesp3)
        ...
        write_sentences("app2_sentencesP4.txt", instances_with_score_and_tag(sentences))
        ...
        write_sentences("app2_sentencesP5.txt", [x.template_text for x in sentences])
...
class GenericTemplateGeneratorApp3(TemplateGenerator):
    ...
    def generate_templates(self, texts_input, relevant_tags, n_masks=2, ranked_words_count=2, min_classification_score=0.9):
        ...
        for instance in instances:
            sentences.extend(instance.split_to_sentences())
        print(f':: {len(sentences)} sentences were generated.')
        write_sentences("app3_sentencesP1.txt", [s.original_text for s in sentences])
        ...
        write_sentences("app3_sentencesP3.txt", [(max(s.prediction.proba), s.original_text) for s in sentences])
        ...
        print(f':: {len(sentences)} sentences remaining.')
        write_sentences("app3_sentencesP4.txt", [(max(s.prediction.proba), s.original_text) for s in sentences])

        # 5. Ranking words by its importance when predicted by target model
        sentences = self.word_ranker.rank(sentences, self.model)
        print(f':: Word ranking done.')
        write_sentences("app3_sentencesP5.txt", instances_with_score_and_tag(sentences))

        # 6. Filtering sentences having {n_masks} relevant words as higher (top-{ranked_words_count}) ranked words
        sentences = RelevantWordsFilter.apply(sentences, relevant_tags, n_masks, ranked_words_count)
        print(f':: {len(sentences)} sentences remaining.')
        write_sentences("app3_sentencesP6.txt", instances_with_score_and_tag(sentences))
        ...
        print(f':: {len(sentences)} sentences remaining.')
        write_sentences("app3_sentencesP7.txt", instances_with_score_and_tag(sentences))


        # 8. Replacing the n most relevant words with masks
        sentences = self.replace_with_masks(sentences, n_masks)
        self.sentences = sentences
        write_sentences("app3_sentencesP8.txt", [x.template_text for x in sentences])
...
class GenericTemplateGeneratorApp4(TemplateGenerator):
    ...
    def generate_templates(self, texts_input, relevant_tags, n_masks=2, ranked_words_count=2, min_classification_score=0.9):
        ...
        write_sentences("app4_sentencesP1.txt", [s.original_text for s in instances])

        # 2. Filter instances unanimously
        instances: list[Instance] = UnanimousClassificationFilter.apply(instances)
        print(f':: {len(instances)} instances remaining.')
        write_sentences("app4_sentencesP2.txt", [(max(s.prediction.proba), s.original_text) for s in instances])
        ...
        for instance in instances:
            sentences.extend(instance.split_to_sentences())
        print(f':: {len(sentences)} sentences were generated.')
        write_sentences("app4_sentencesP3.txt", [s.original_text for s in sentences])
        ...
        sentences = UnanimousClassificationFilter.apply(sentences)
        print(f':: {len(sentences)} sentences remaining.')
        write_sentences("app4_sentencesP5.txt", [(max(s.prediction.proba), s.original_text) for s in sentences])

        # 6. Filtering sentences with high average score in classification step
        sentences = HighClassificationScoreFilter.apply(sentences, min_classification_score)
        print(f':: {len(sentences)} sentences remaining.')
        write_sentences("app4_sentencesP6.txt", [(max(s.prediction.proba), s.original_text) for s in sentences])

        # 7. Ranking words by its importance when predicted by target model
        sentences = self.word_ranker.rank(sentences, self.model)
        print(f':: Word ranking done.')
        write_sentences("app4_sentencesP7.txt", instances_with_score_and_tag(sentences))

        # 8. Filtering sentences having {n_masks} relevant words as higher (top-{ranked_words_count}) ranked words
        sentences = RelevantWordsFilter.apply(sentences, relevant_tags, n_masks, ranked_words_count)
        print(f':: {len(sentences)} sentences remaining.')
        write_sentences("app4_sentencesP8.txt", instances_with_score_and_tag(sentences))

        # 9. Filtering sentences having relevant words with high score
        sentences = HighClassificationScoreWordFilter.apply(sentences, self.model, relevant_tags, n_masks, ranked_words_count, 
                                                            min_classification_score)
        print(f':: {len(sentences)} sentences remaining.')
        write_sentences("app4_sentencesP9.txt", instances_with_score_and_tag(sentences))

        # 10. Replacing the n most relevant words with masks
        sentences = self.replace_with_masks(sentences, n_masks)
        self.sentences = sentences
        write_sentences("app4_sentencesP10.txt", [x.template_text for x in sentences])
...
class GenericTemplateGeneratorApp5(TemplateGenerator):
    ...
    def generate_templates(self, texts_input, relevant_tags, n_masks = 2, ranked_words_count=2, min_classification_score=0.9):
        ...
        for instance in instances:
            sentences.extend(instance.split_to_sentences())
        print(f':: {len(sentences)} sentences were generated.')
        write_sentences("app5_sentencesP1.txt", [s.original_text for s in sentences])

        # 2. Ranking words by its importance when predicted by target model
        sentences = self.word_ranker.rank(sentences, self.model)
        print(f':: Word ranking done.')
        write_sentences("app5_sentencesP2.txt", ["length: " + str(s.length) + "\t" + s.original_text for s in sentences])
        # 3. Filter by sentence size
        sentences = MinimmumInputSizeFilter.apply(sentences, 5)
        print(f':: {len(sentences)} sentences remaining.')
        write_sentences("app5_sentencesP3.txt", instances_with_score_and_tag(sentences))

        # 4. Filtering sentences having only relevant words as higher ranked words
        sentences = RelevantWordsFilter.apply(sentences, relevant_tags, n_masks, ranked_words_count)
        print(f':: {len(sentences)} sentences remaining.')
        write_sentences("app5_sentencesP4.txt", instances_with_score_and_tag(sentences))

        # 5. Filtering sentences having relevant words with high score
        sentences = HighClassificationScoreWordFilter.apply(sentences, self.model, relevant_tags, n_masks, ranked_words_count, 
                                                            min_classification_score)
        print(f':: {len(sentences)} sentences remaining.')
        write_sentences("app5_sentencesP5.txt", instances_with_score_and_tag(sentences))
        ...
        sentences = self.replace_with_masks(sentences, n_masks)
        self.sentences = sentences
        write_sentences("app5_sentencesP7.txt", [x.template_text for x in sentences])
